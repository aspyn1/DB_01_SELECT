/*
 * SELCET(DML 또는 DQL) : 조회
 * 
 * - 데이터를 조회(SELECT)하면 조건에 맞는 행들이 조회됨.
 * 이때, 조회된 행들의 집합을 "RESULT SET"(조회결과의 집합)이라고 한다.
 * 
...
-- 2023-08-06 12:32:41.000
-- SYSDATE : 시스템상의 현재시간(날짜)를 나타내는 상수(변하지않음)
...
FROM DUAL;
-- 날짜에 +/- 연산 시 일 단위 계산이 진행됨

-------------------------------------------------------------------

-- <컬럼 별칭 지정>
/* 컬럼명 AS 별칭 : 별칭 띄어쓰기 X, 특수문자 X, 문자만 ㅇ
 * 
 * 컬럼명 AS "별칭" : 별칭 띄어쓰기 ㅇ, 특수문자 ㅇ, 문자만 ㅇ
 * 
 * AS 생략 가능
 * */
...
-- 주의사항 1) DISTINCT 구문은 SELECT 마다 딱 한번씩만 작성가능함
-- 주의사항 2) DISTINCT 구문은 SELECT 제일 앞에 작성 되어야 한다.
...
-- ORDER BY : ORDER BY 컬럼명 || 별칭 || 컬럼순서 [ASC (오름차순)/ DESC (내림차순)] [NULLS FIRST || LAST]

-- 해석순서 : FROM -> WHERE -> SELECT -> ORDER BY

--> EMPLOYEE 테이블에서 급여가 3백만원 초과인 사원의 사번, 이름, 급여, 부서코드를 조회

SELECT EMP_ID, EMP_NAME, SALARY, EMP_CODE
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 입사일이 1990-01-01 ~ 1999-12-31사이인 직원의
-- 이름, 입사일 조회
...
-- EMPLOYEE 테이블에서 이메일의 _앞의 글자가 세글자인 사원만 조회

-- ESCAPE 문자
-- ESCAPE 문자뒤에 작성된 _는 일반 문자로 탈출한다는 뜻
-- #,^를 이용하여 사용함
...
-- EMPLOYEE 테이블에서
-- 이메일 '_' 앞이 4글자 이면서
-- 부서코드가 'D9' 또는 'D6'이고 -> AND가 OR보다 우선순위가 높아 ()사용
...
/* ORDER BY 절
 * 
 * - SELECT문의 조회 결과(RESULT SET)을 정렬할때 사용하는 구문
 * ** SELCET문 해석 시 가장 마지막에 해석된다!!
 * 
 * [해석순서]
 * 3. SELECT절
 * 1. FROM
...
ORDER BY 3 DESC; -- 내가 조회할 컬럼의 3번째(급여) 조회
