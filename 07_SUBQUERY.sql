...
FROM EMPLOYEE
WHERE SALARY >= 3047663; -- 선동일, 송종기, 노옹철, 유재식, 정중하, 심봉선, 대북혼, 전지연
...
WHERE SALARY > (SELECT AVG(SALARY)
				FROM EMPLOYEE)
ORDER BY JOB_CODE;
-- SELECT 절에 없는 컬럼이라도 FROM, JOIN으로 인해 존재하는 컬럼이면 
-- ORDER BY 절에서 사용 가능
...
WHERE (DEPT_CODE, JOB_CODE ) = (SELECT DEPT_CODE, JOB_CODE
								...
								--WHERE EXTRACT(YEAR FROM HIRE_DATE) = 2000 도 가능
								);
...
SELECT EMP_NAME, DEPT_CODE, SALARY, (SELECT MAX(SALARY) FROM EMPLOYEE) - SALARY
FROM EMPLOYEE;
...
-- WHERE DEPT_TITLE = '기술지원부'; DEPT_TITLE 사용못함
WHERE 부서 = '기술지원부';
...
-- ROWNUM 컬럼 : 행번호를 만들어주는 가상 컬럼
-- SELECT, WHERE, ORDER BY절에서 사용 가능

-- 1) 이름, 급여를 급여 내림차순으로 조회한 결과를 인라인뷰로 사용
--> FROM절에 작성되기 때문에 해석순위 1순위
-- 2) 메일쿼리 조회 시 ROWNUM을 5이하까지만 조회
