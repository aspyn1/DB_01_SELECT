 ...
 * 		CREATE TABLE 		: TABLE 생선 권한
 * 		CREATE VIEW 		: 뷰 생성 권한
 * 		CREATE SEQUENCE 	: 시퀀스 생성 권한
 * 		CREATE PROCEDURE	: 함수(PROCEDUR) 생성 권한
...
-- 2. 새 연결(접속방법) 추가
--> CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
--> 접속 권한 필요
...
--(sys) smaple 계정에 커넥트, 리소스 부여
GRANT CONNECT, RESOURCE TO cje_sample;

-- CONNECT : DB접속 관련 권한을 묶어둔 ROLE
-- RESOURCE : DB사용을 위한 기본 객체 생성 권한을 묶어둔 ROLE
...
-- kh / cje_sample 사용자 계정끼리 서로 객체에 접근할 수 있는 권한 부여
...
-- 2. (kh) cje_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO cje_sample;

-- 3. (sample) 다시 kh계정의 EMPLOYEE테이블 조회
SELECT * FROM kh.EMPLOYEE;
...
REVOKE SELECT ON EMPLOYEE FROM CJE_SAMPLE;
