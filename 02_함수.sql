...
-- 그룹(GROUP BY) 함수 : N개의 값을 읽어서 1개의 결과를 반환 (합계, 평균, 최소, 최대)
...
-- 지정한 위치부터 지정한 순번째로 검색되는 문자의 위치를반환
...
-- EMPLOYEE테이블에서 사원명, 이메일 중 아이디만 조회
...
SELECT EMP_ID, SALARY, MOD (SALARY, 1000000)
FROM EMPLOYEE; 
...
SELECT TRUNC(123.456) FROM DUAL; -- 소수점 아래를 버림 / 123
SELECT TRUNC(123.456, 2) FROM DUAL; -- 소수점 둘째자리 아래까지 버림 / 123.45
SELECT TRUNC(123.456, -1) FROM DUAL; -- 10의 자리 아래까지 버림 / 120
...
SELECT ABS( ROUND (MONTHS_BETWEEN(SYSDATE, '2023-12-22'), 1) ) "수강 기간(개월)"
FROM DUAL; -- 4.5
...
SELECT LAST_DAY('2020-12-01') FROM DUAL;


-- EXTRACT : 년, 월, 일 정보를 추출하여 리턴
-- EXTRACT(YEAR FROM 날짜) : 년도만 추출
-- EXTRACT(MOTH FROM 날짜) : 월만 추출
-- EXTRACT(DAY FROM 날짜) : 일만 추출

-- EMPLOYEE 테이블에서 
-- 각 사원의 이름, (입사년도, 월, 일) 입사일 조회
-- 입사일
-- 2023년 08월 04일
...
SELECT TO_CHAR(1234) FROM DUAL; -- 문자열 '1234'
...
-- 년, 월, 일이 날짜를 나타내는 패턴으로 인식이 안돼서 오류발생
-- 년, 월, 일을 ""로 묶어 단순한 문자로 인식시켜야함
...
SELECT TO_DATE('20220902 101835', 'YYMMDD HH24MISS') FROM DUAL;
--> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인지 시킴

-- Y 패턴 : 현재 세기(21세기 == 20XX년 == 2000 년대)
-- R 패턴 : 1세기를 기준으로 절반(50년) 이상인 경우에는 이전세기 (1900년대)
-- 			절반(50년) 미만인 경우 현재 세기(2000년대)

SELECT TO_DATE('500505', 'YYMMDD') FROM DUAL; -- 2050-05-05 00:00:00.000
SELECT TO_DATE('500505', 'RRMMDD') FROM DUAL; -- 1950-05-05 00:00:00.000
SELECT TO_DATE('490505', 'RRMMDD') FROM DUAL; -- 2049-05-05 00:00:00.000


-- EMPLOYEE 테이블에서 각직원이 태어난 생년월일 조회
SELECT EMP_NAME, 
...
-- EMPLOYEE 테이블에서 보너스를 받는사람이면 '0', 안받으면 'X'조회
...
-- COUNT([DISTINCT] 컬럼명) : 중복 제거한 행 개수를 헤아려서 리턴
